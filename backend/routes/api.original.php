...
// Simple login route for frontend testing
...
Route::prefix('v1')->group(function () {
    ...
    // Autenticazione (solo login pubblico, registrazione rimossa)
    Route::prefix('auth')->group(function () {
        Route::post('login', [AuthController::class, 'login']);
        // Route::post('register', [AuthController::class, 'register']); // RIMOSSA - Solo admin può creare utenti
    ...
    Route::get('health', function() {
        return response()->json([
            'status' => 'ok',
            'timestamp' => now(),
            'service' => 'NexioSolution API'
        ]);
    });
});

// Route semplificate per compatibilità frontend (senza versioning)
// Utilizziamo SimpleAuthController per gestione più semplice
Route::post('login', [SimpleAuthController::class, 'simpleLogin']);
// Route::post('register', [SimpleAuthController::class, 'simpleRegister']); // RIMOSSA - Solo admin può creare utenti

// Route con autenticazione Sanctum per frontend
Route::middleware('auth:sanctum')->group(function () {
    Route::get('user', [SimpleAuthController::class, 'simpleUser']);
    Route::post('logout', [SimpleAuthController::class, 'simpleLogout']);
